...
export default function Profiles() {
  ...
  async function getProfiles(): Promise<void> {
    ...
    setProfiles(docSnap.data().accounts);
  }
  ...
  return (
    <div className="profiles">
      <h1 className="profiles__title">Manage Profiles:</h1>
      <ul className="profiles__container">
        {profiles.map((profile, index) => (
          <li className="profiles__profile-box" key={index}>
            <div className="profiles__icon-container">
              <Image className="profiles__profile-icon" src={getAvatar(profile)} alt="Profile"/>
              <div className="profiles__edit-image-border">
                <svg className="profiles__edit-image" viewBox="0 0 24 24" fill="none" data-mirrorinrtl="true" data-name="Pencil" aria-labelledby=":r7:" aria-hidden="true">
            ...
            <div className="profiles__profile-name">{profile.name}</div>
          </li>
        ))}
      </ul>
      <Link className="profiles__done-button" href={'/browse'}>Done</Link>
    </div>
  )
}